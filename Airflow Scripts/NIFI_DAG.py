...
class NewFolderSensor(BaseSensorOperator):
    ...
    def poke(self, context):
        ...
        # Obtener lista inicial (primera ejecución)
...
default_args = {
    'owner': 'airflow',
    'start_date': datetime(2025, 7, 2, 15, 00), # Desired time 3/7/25 8:00am = Airflow time 2/7/2025 15:00pm (sum 7 hours minus a day)
...
#-----------------GET TOKEN-----------------------------------------------------------
    ...
    get_token = BashOperator(
        ...
        do_xcom_push=True,  # Guarda el output en XCom 
    )
...
#------------------RUN AND STOP PG 1-------------------------------------------------
    ...
    folder_appeared_pg1 = NewFolderSensor(
        ...
        hdfs_path='/user/kerberos/TEST_ETL/STAGING_BUCKET/sucursalez',
        timeout=180,  
        poke_interval=15,
        mode='poke'
    )
...
#-----------------TRIGGER NEXT DAG-----------------------------------------------------------
    run_dag2 = TriggerDagRunOperator(
        task_id='run_dag2',
        trigger_dag_id='SPARK_SUBMIT_DAGx',
        execution_date='2025-07-02T15:00:00+00:00',
        reset_dag_run=True,
